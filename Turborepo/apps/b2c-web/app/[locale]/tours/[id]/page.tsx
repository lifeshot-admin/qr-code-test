...
import {
  ...
  Share2, Heart, Star, Camera, AlertTriangle, Sparkles,
} from "lucide-react";
...
export default function TourDetailPage() {
  ...
  const handleReserve = useCallback(async () => {
    ...
    if (!canReserve || !selectedSchedule || checkingAvailability) return;
    ...
    // 세션 확인 (unauthenticated → getSession 재확인)
    ...
    try {
      console.log(`🔍 [RESERVE_CHECK] 스케줄 검증 시작 — scheduleId: ${selectedSchedule.id}`);
      const checkRes = await fetch(`/api/backend/check-schedule-availability?scheduleId=${selectedSchedule.id}`);
      const checkData = await checkRes.json();

      if (!checkData.available) {
        console.warn("⚠️ [RESERVE_CHECK] 중복 예약 감지:", checkData.reason);
        setToast(null);
        setTimeout(() => {
          setToast(checkData.reason || "이미 예약된 스케줄이 있습니다. 다른 시간을 선택해주세요.");
          setTimeout(() => setToast(null), 4000);
        }, 50);
        setCheckingAvailability(false);
        return;
      }
      console.log("✅ [RESERVE_CHECK] 스케줄 검증 통과 → GuestSheet 열기");
    } catch (e) {
      console.warn("⚠️ [RESERVE_CHECK] 검증 API 호출 실패 — 예약 진행 허용:", e);
    } finally {
      setCheckingAvailability(false);
    }

    console.log(`✅ [RESERVE_CHECK] 인증 통과 → role="${activeSession?.user?.role}", GuestSheet 열기`);
    setGuestSheetOpen(true);
  }, [canReserve, selectedSchedule, sessionStatus, session, router, checkingAvailability]);
  ...
  if (error || !tour) return (
    <div className="min-h-screen bg-white flex items-center justify-center px-6">
      <div className="text-center max-w-sm">
        ...
        <button onClick={() => router.back()} className="mt-4 px-6 py-3 bg-cheiz-primary text-white font-semibold rounded-xl text-sm">뒤로 가기</button>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen bg-white pb-28">
        ...
        {toast && (
          ...
          >
            <div className="bg-[#1A1A1A] text-white text-sm font-semibold px-5 py-3 rounded-xl shadow-lg flex items-center gap-2">
              <span className={toast.includes("이미") || toast.includes("선택해") ? "text-yellow-400" : "text-green-400"}>
                {toast.includes("이미") || toast.includes("선택해") ? "⚠️" : "✓"}
              </span>
              {toast}
            </div>
          </motion.div>
      ...
      <div className="max-w-md mx-auto px-5 pt-5">
        {locationLabel && (
          <div className="flex items-center gap-1.5 mb-2">
            <MapPin className="w-3.5 h-3.5 text-cheiz-primary" />
            <span className="text-xs font-medium text-cheiz-primary">{locationLabel}</span>
          </div>
        )}
          ...
          <div className="space-y-2">
            {descriptionLines.map((line, i) => (
              <div key={i} className="flex items-start gap-2">
                <span className="w-1.5 h-1.5 rounded-full bg-cheiz-primary flex-shrink-0 mt-[7px]" />
      ...
      <div className="max-w-md mx-auto px-5 pt-5">
        <h2 className="text-base font-bold text-gray-900 mb-3 flex items-center gap-2">
          <Calendar className="w-4 h-4 text-cheiz-primary" />
          예약 가능 일정
          {schedules.length > 0 && (
              ...
              return (
                <motion.button
                  ...
                  className={`
                    ...
                    ${isSelected
                      ? "bg-cheiz-primary text-white border-cheiz-primary shadow-lg shadow-cheiz-primary/20"
                      : isFull
                      ? "bg-gray-50 text-gray-300 border-gray-100"
                      : "bg-white text-gray-700 border-gray-100 hover:border-cheiz-primary/40 hover:bg-blue-50/50"
                    }
                  `}
                >
                  ...
                  <p className={`text-xs font-bold mb-2 ${isSelected ? "text-cheiz-primary" : isFull ? "text-gray-300" : "text-gray-400"}`}>
                    ({dt.day})
                  </p>
                  ...
                  {s.remainingCapacity !== undefined && s.remainingCapacity !== null && (
                    <p className={`text-[10px] mt-1.5 font-bold ${isSelected ? "text-cheiz-primary" : isFull ? "text-gray-300" : "text-cheiz-primary"}`}>
      ...
      <div className="max-w-md mx-auto px-5">
        <h2 className="text-base font-bold text-gray-900 mb-3 flex items-center gap-2">
          <Star className="w-4 h-4 text-cheiz-primary" />
          이 상품을 이용한 리뷰
        </h2>
        ...
        {reviews.length === 0 ? (
          ...
          <div className="space-y-3">
            {reviews.map((r, i) => {
              ...
              const isCorrected =
                r.color_grade_status === "completed" &&
                Array.isArray(r.corrected_images) &&
                r.corrected_images.length > 0;
              const isPending = r.color_grade_status === "pending";
              const thumbUrl = isCorrected
                ? r.corrected_images[0]
                : r.image;
              const personaTag = formatPersona(r.persona, r.guest_count, appLang);
              const rText = r.text || r.review || "";
              const rRating = r.rating ?? r.score;
                ...
                <motion.div key={r._id || i} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
                  ...
                  <div className="flex">
                    {isPending && !isCorrected ? (
                      <div className="w-20 h-20 flex-shrink-0 overflow-hidden relative bg-gradient-to-br from-cheiz-primary/8 via-purple-50 to-pink-50">
                        <div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/40 to-transparent animate-shimmer" />
                        <div className="absolute inset-0 flex items-center justify-center z-10">
                          <Sparkles className="w-4 h-4 text-cheiz-primary animate-sparkle-1" />
                        </div>
                        <Sparkles className="absolute top-1.5 right-1.5 w-2 h-2 text-purple-300/60 animate-sparkle-2" />
                      </div>
                    ) : thumbUrl ? (
                      <div className="w-20 h-20 flex-shrink-0 bg-gray-200 bg-cover bg-center relative"
                        style={{ backgroundImage: `url(${thumbUrl.startsWith("//") ? `https:${thumbUrl}` : thumbUrl})` }}>
                        {isCorrected && (
                          <div className="absolute bottom-0 inset-x-0 bg-gradient-to-t from-black/50 to-transparent px-1 pb-0.5 pt-2">
                            <span className="text-[7px] text-white/90 font-medium flex items-center gap-0.5">
                              <Sparkles className="w-2 h-2" /> 보정
                            </span>
                          </div>
                        )}
                      </div>
                    ...
                    <div className="p-3 flex-1 min-w-0">
                      {rRating != null && (
                        <div className="flex items-center gap-0.5 mb-1">
                          {[...Array(5)].map((_, j) => (
                            <Star key={j} className={`w-3 h-3 ${j < rRating ? "fill-yellow-400 text-yellow-400" : "text-gray-200"}`} />
                          ))}
                        </div>
                      )}
                      {rText && (
                        <p className="text-xs text-gray-600 line-clamp-2 mb-1">&ldquo;{rText}&rdquo;</p>
                      )}
                      <div className="flex items-center gap-1.5">
                        <span className="text-[10px] text-gray-400 whitespace-nowrap">- {nick}</span>
                        {personaTag && (
                          <span className="text-[9px] text-cheiz-sub whitespace-nowrap">{personaTag}</span>
                        )}
                      </div>
                    </div>
                  </div>
      ...
      <div className="max-w-md mx-auto px-5">
        <h2 className="text-base font-bold text-gray-900 mb-3 flex items-center gap-2">
          <MapPin className="w-4 h-4 text-cheiz-primary" />
          촬영 장소 안내
        </h2>
        ...
        {photographerLocationImages.length > 0 && (
          <div className="mb-5">
            <h3 className="text-sm font-bold text-gray-800 mb-3 flex items-center gap-1.5">
              <Camera className="w-3.5 h-3.5 text-cheiz-primary" />
              촬영 장소 사진
            </h3>
        ...
        {/* 포토그래퍼 안내 */}
        <div className="bg-gradient-to-br from-cheiz-primary/5 to-blue-50 rounded-2xl p-5 text-center">
          <div className="w-14 h-14 mx-auto mb-3 rounded-full bg-cheiz-primary/10 flex items-center justify-center">
            <Camera className="w-7 h-7 text-cheiz-primary" />
          </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 bg-white border-t border-gray-100">
        <div className="max-w-md mx-auto px-5 py-4 flex items-center gap-3">
          ...
          <motion.button
            whileTap={canReserve && !alreadyReserved && !checkingAvailability ? { scale: 0.97 } : undefined}
            onClick={handleReserve}
            disabled={!canReserve || tour.isClosed || sessionStatus === "loading" || alreadyReserved || checkingAvailability}
            className={`px-8 py-3.5 rounded-xl font-semibold text-sm transition-all duration-200 flex-shrink-0 flex items-center gap-2
              ...
              checkingAvailability ? "bg-cheiz-primary/70 text-white cursor-wait" :
              canReserve && !tour.isClosed && sessionStatus !== "loading" ? "bg-cheiz-primary text-white shadow-sm active:bg-cheiz-dark" : "bg-gray-200 text-gray-400 cursor-not-allowed"}`}
          >
            {sessionStatus === "loading" || checkingAvailability ? (
              <><svg className="animate-spin w-4 h-4" viewBox="0 0 24 24" fill="none"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4" /><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z" /></svg>{checkingAvailability ? "확인 중..." : "확인 중"}</>
...
function MapSection({ tour }: { tour: TourDetail }) {
  ...
  const mapBadge = (
    <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-10">
      <span className="bg-white/95 backdrop-blur-sm text-xs font-semibold text-cheiz-primary px-4 py-2 rounded-full shadow-md flex items-center gap-1.5">
