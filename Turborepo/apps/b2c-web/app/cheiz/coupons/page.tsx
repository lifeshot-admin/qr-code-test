    ...
    return (
      <div className="min-h-screen bg-[#FAFAF8] flex items-center justify-center">
        <div className="text-center">
          <div className="animate-spin rounded-full h-12 w-12 border-t-3 border-cheiz-primary border-solid mx-auto mb-4" />
      ...
      <div className="bg-white border-b border-gray-100 sticky top-0 z-40">
        <div className="max-w-md mx-auto px-5 py-3 flex items-center justify-between">
          <button onClick={() => router.back()}
            className="text-gray-500 hover:text-cheiz-primary transition-colors text-sm flex items-center gap-1 active:scale-95">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="flex gap-2">
            <input type="text" value={couponCode}
              ...
              className="flex-1 h-12 px-4 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-cheiz-primary focus:ring-1 focus:ring-cheiz-primary/20 transition-all placeholder:text-gray-400" />
            <button onClick={handleRegisterCoupon} disabled={registerLoading || !couponCode.trim()}
              className="h-12 px-5 bg-cheiz-primary text-white text-sm font-bold rounded-xl disabled:opacity-40 hover:bg-opacity-90 transition-all active:scale-95 flex-shrink-0">
              {registerLoading ? "..." : "등록"}
            </button>
          </div>
          {registerMsg && (
              ...
              <motion.div key={c.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
                ...
                <div className="p-4 pb-3">
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    {c.aiCount > 0 && (
                      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-cheiz-surface rounded-lg">
                        <Sparkles className="w-3.5 h-3.5 text-cheiz-primary" />
                        <span className="text-xs font-bold text-cheiz-dark">AI 전체 보정권</span>
                        <span className="text-xs font-extrabold text-cheiz-text">{c.aiCount}장</span>
                      </div>
                    )}
                    {c.retouchCount > 0 && (
                ...
                <div className="border-t border-dashed border-gray-200 px-4 py-3">
                  <button onClick={() => handleRedeem(c.code, c.id)}
                    ...
                    className="w-full py-2.5 rounded-xl bg-cheiz-primary text-white text-sm font-bold disabled:opacity-50 hover:bg-opacity-90 active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                    {redeemLoading === c.id ? (
