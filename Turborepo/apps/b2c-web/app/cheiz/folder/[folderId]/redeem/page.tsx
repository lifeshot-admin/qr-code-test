        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }}
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <div className="px-5 py-4 space-y-2.5">
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className="w-7 h-7 rounded-lg bg-cheiz-primary/20 flex items-center justify-center">
          ...
          <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
            ...
            <button onClick={() => router.push(`/cheiz/folder/${folderId}`)}
              className="flex-1 py-3.5 rounded-2xl bg-cheiz-primary text-white font-bold text-sm active:scale-[0.97] transition-all shadow-lg shadow-cheiz-primary/30 flex items-center justify-center gap-2">
      ...
      <div className="max-w-md mx-auto px-5 pt-5 space-y-4">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }}
          ...
          {user?.image ? (
            ...
            <div className="w-11 h-11 rounded-full bg-cheiz-primary/10 flex items-center justify-center">
              <User className="w-5 h-5 text-cheiz-primary" />
            </div>
          )}
              ...
              {photoIds.map((id, i) => {
                ...
                return (
                  <div key={id} className="relative flex-shrink-0 w-14 h-14 rounded-lg overflow-hidden bg-gray-100">
                    ...
                    <div className="absolute bottom-0 right-0 bg-cheiz-primary text-white text-[8px] font-bold w-4 h-4 rounded-tl-md flex items-center justify-center z-[5]">{i + 1}</div>
                  </div>
                );
              })}
            </div>
                  ...
                  <div className="px-4 pb-4 space-y-3">
                    {credits.photo > 0 && (
                      <CreditStepper label={`사진 크레딧 (보유 ${credits.photo}장)`} icon={Camera}
                        color="bg-cheiz-surface text-cheiz-primary" value={photoCreditsUse} max={maxPhotoCredits} onChange={setPhotoCreditsUse} />
                    )}
                    ...
                    <button onClick={() => router.push("/cheiz/coupons")}
                      className="w-full py-2.5 rounded-xl border border-dashed border-cheiz-primary/30 bg-cheiz-primary/5 text-cheiz-primary text-xs font-bold active:scale-[0.98] transition-all flex items-center justify-center gap-1.5 hover:bg-cheiz-primary/10">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.09 }}
          className="bg-gradient-to-br from-cheiz-primary to-[#3377FF] rounded-2xl p-5 text-white">
        ...
        <div className="max-w-md mx-auto px-5 py-3">
          {creditsLoading ? (
            ...
            <button onClick={handlePayment} disabled={processing || verifying}
              className="w-full h-14 bg-cheiz-primary text-white text-base font-bold rounded-2xl disabled:opacity-60 active:scale-[0.98] transition-all flex items-center justify-center gap-2 shadow-lg shadow-cheiz-primary/20">
