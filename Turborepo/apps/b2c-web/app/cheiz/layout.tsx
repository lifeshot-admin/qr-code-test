...
export default function CheizLayout({
  ...
  // ━━━ 메뉴바 숨김 경로: 예약 플로우 + 폴더 + 리터처 + 이벤트 상세 + 앨범 상세 + 리뷰 작성 ━━━
  ...
  const isReviewWrite = pathname.startsWith("/cheiz/reviews");
  const isEventDetail = /^\/cheiz\/events\/.+/.test(pathname);
  const isAlbumDetail = /^\/cheiz\/albums\/.+/.test(pathname);
  const isImmersive = isReserveFlow || isFolderFlow || isRetoucherPage || isReviewWrite || isEventDetail || isAlbumDetail;
