...
function CreditBenefitItem({ count, label }: { count: number; label: typeof CREDIT_LABELS.photo }) {
  ...
  return (
    <div className="flex items-start gap-2.5 py-1.5">
      <div className="w-5 h-5 rounded-md bg-cheiz-primary/10 flex items-center justify-center flex-shrink-0 mt-0.5">
        <Check className="w-3 h-3 text-cheiz-primary" />
      </div>
      <div>
        <p className="text-sm font-semibold text-gray-800">{label.name} <span className="text-cheiz-primary">{count}{count > 1 ? "장" : "회"}</span></p>
...
function CouponDetailCard({
  ...
  return (
    <motion.div
      ...
      className="mt-4 bg-gradient-to-br from-white to-cheiz-surface/50 rounded-2xl border border-cheiz-primary/15 shadow-lg shadow-cheiz-primary/5 overflow-hidden"
    >
      {/* 상단 헤더 */}
      <div className="bg-gradient-to-r from-cheiz-primary to-[#3377FF] px-5 py-3.5 flex items-center justify-between">
      ...
      <div className="px-5 pb-5 flex gap-2">
        ...
        <button
          onClick={onConfirm}
          disabled={loading}
          className="flex-[2] py-3 text-sm font-bold bg-cheiz-primary text-white rounded-xl hover:bg-opacity-90 transition-all disabled:opacity-50 flex items-center justify-center gap-1.5"
        >
          {loading ? (
...
function CheckoutContent() {
  ...
  const couponTypeInfo = (type: string) => {
    ...
    if (t === "PHOTO" || t === "DOWNLOAD" || t === "SNAP" || t === "SNAP_DOWNLOAD") return { label: CREDIT_LABELS.photo.short, icon: <Download className="w-3.5 h-3.5" />, color: "text-cheiz-primary" };
    if (t === "AI" || t === "AI_RETOUCH") return { label: CREDIT_LABELS.ai.short, icon: <Sparkles className="w-3.5 h-3.5" />, color: "text-cheiz-primary" };
    ...
    <div className="min-h-screen bg-[#FAFAF8] pb-36">
      ...
      <div className="bg-white sticky top-0 z-40 border-b border-gray-100">
        <div className="max-w-md mx-auto px-5 py-3 flex items-center justify-between">
          <button onClick={() => router.back()} className="text-gray-500 hover:text-cheiz-primary text-sm flex items-center gap-1">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 space-y-3">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-cheiz-primary/10 flex items-center justify-center"><Users className="w-5 h-5 text-cheiz-primary" /></div>
          ...
          <div className="border-t border-gray-100 pt-3 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <div className="w-10 h-10 rounded-xl bg-cheiz-surface flex items-center justify-center"><Camera className="w-5 h-5 text-cheiz-primary" /></div>
      ...
      <div className="max-w-md mx-auto px-5 py-2">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          <div onClick={() => setAiRetouching(!safeAiRetouching)} className={`flex items-center justify-between cursor-pointer transition-all rounded-xl p-3 -m-1 ${safeAiRetouching ? "bg-cheiz-primary/5" : "bg-transparent"}`}>
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-xl flex items-center justify-center ${safeAiRetouching ? "bg-cheiz-primary/10" : "bg-gray-100"}`}>
                <Sparkles className={`w-5 h-5 ${safeAiRetouching ? "text-cheiz-primary" : "text-gray-400"}`} />
              </div>
              <div>
                <p className={`text-sm font-semibold ${safeAiRetouching ? "text-cheiz-primary" : "text-gray-700"}`}>{CREDIT_LABELS.ai.name}</p>
            ...
            <div className="flex items-center gap-3">
              ...
              <div className={`w-12 h-[26px] rounded-full flex items-center transition-all ${safeAiRetouching ? "bg-cheiz-primary justify-end" : "bg-gray-300 justify-start"}`}>
      ...
      <div className="max-w-md mx-auto px-5 py-2">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.2 }} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          <h4 className="text-sm font-bold text-gray-900 mb-4 flex items-center gap-1.5">
            <Gift className="w-4 h-4 text-cheiz-primary" /> 나의 보유 크레딧
          </h4>
          ...
          <div className="space-y-2 mb-4 bg-gray-50 rounded-xl p-3.5">
            <div className="flex items-start gap-2 text-[11px] text-gray-500">
              <Download className="w-3 h-3 text-cheiz-primary flex-shrink-0 mt-0.5" />
              <span><b className="text-gray-700">{CREDIT_LABELS.photo.name}</b> — {CREDIT_LABELS.photo.detailDescription}</span>
            </div>
            <div className="flex items-start gap-2 text-[11px] text-gray-500">
              <Sparkles className="w-3 h-3 text-cheiz-primary flex-shrink-0 mt-0.5" />
          ...
          <div className="border-t border-gray-100 pt-4 mt-4">
            ...
            <div className="flex gap-2">
              <input type="text" value={couponCode}
                ...
                className="flex-1 h-11 px-4 border border-gray-200 rounded-xl text-sm focus:outline-none focus:border-cheiz-primary transition-colors placeholder:text-gray-400" />
              <button onClick={() => handlePreviewCoupon()} disabled={couponLoading || !couponCode.trim()}
                className="h-11 px-5 bg-cheiz-primary text-white text-sm font-bold rounded-xl disabled:opacity-40 hover:bg-opacity-90 transition-all flex-shrink-0">
                {couponLoading ? "..." : "조회"}
              </button>
            </div>
      ...
      <div className="max-w-md mx-auto px-5 py-2">
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.3 }} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-5">
          ...
          <div className="flex items-center gap-3 text-xs text-gray-500">
            <a href="https://www.lifeshot.me/version-test/privacy" target="_blank" rel="noopener noreferrer" className="text-cheiz-primary hover:underline flex items-center gap-0.5">개인정보처리방침 <ExternalLink className="w-3 h-3" /></a>
            <span className="text-gray-300">|</span>
            <a href="https://lifeshot.notion.site" target="_blank" rel="noopener noreferrer" className="text-cheiz-primary hover:underline flex items-center gap-0.5">이용약관 <ExternalLink className="w-3 h-3" /></a>
          </div>
        </motion.div>
      </div>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white/95 backdrop-blur-md border-t border-gray-100 z-50">
        <div className="max-w-md mx-auto px-5 py-4">
          ...
          <button onClick={handleCheckout} disabled={processing}
            className={`w-full py-4 rounded-2xl font-bold text-base flex items-center justify-center gap-2 transition-all ${processing ? "bg-gray-200 text-gray-400 cursor-not-allowed" : "bg-cheiz-primary text-white shadow-lg shadow-cheiz-primary/25 active:scale-[0.98]"}`}>
...
export default function CheckoutPage() {
  return (
    <Suspense fallback={<div className="min-h-screen bg-[#FAFAF8] flex items-center justify-center"><div className="animate-spin rounded-full h-16 w-16 border-t-4 border-cheiz-primary border-solid" /></div>}>
      <CheckoutContent />
    </Suspense>
  );
}
