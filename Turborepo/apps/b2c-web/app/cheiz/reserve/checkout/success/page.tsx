    ...
    try {
      ...
      if (editMode && existingReservationId) {
        try {
          ...
          console.log("[CHECKOUT_SUCCESS] ✅ 기존 예약 상태 → CANCELED");
        } catch (e) {
          console.warn("[CHECKOUT_SUCCESS] ⚠️ 기존 예약 상태 변경 실패:", e);
        }
      }
      ...
      // ━━━ STEP 4: Bubble 알림 생성 (PAYMENT_COMPLETE) ━━━
      try {
        await fetch("/api/bubble/notifications", {
          ...
          body: JSON.stringify({
            ...
            title: "결제가 완료되었습니다",
            body: tour?.tour_name || "투어 결제",
            link_id: finalFolderId ? Number(finalFolderId) : null,
            is_read: false,
          }),
        });
        console.log("[CHECKOUT_SUCCESS] ✅ PAYMENT_COMPLETE 알림 생성 완료");
      } catch (notifErr) {
        console.warn("[CHECKOUT_SUCCESS] ⚠️ 알림 생성 실패 (비치명적):", notifErr);
      }
      ...
      <div className="min-h-screen bg-[#FFF9F5] flex items-center justify-center px-6">
        ...
        >
          ...
          <button
            ...
            className="w-full py-3 rounded-2xl bg-cheiz-primary text-white font-bold"
          >
            마이페이지로 이동
          </button>
        </motion.div>
  ...
  return (
    <div className="min-h-screen bg-[#FFF9F5] flex items-center justify-center px-6">
      ...
      >
        ...
        <h2 className="text-2xl font-bold text-cheiz-primary mb-2">
          예약 완료!
        </h2>
        ...
        {/* 6자리 예약 코드 */}
        {reservationCode && (
          <div className="bg-gradient-to-r from-cheiz-primary/10 to-[#7B2BFF]/10 border-2 border-cheiz-primary/20 rounded-2xl p-5 mb-6 text-center">
        ...
        <div className="flex flex-col gap-3">
          <button
            onClick={() => router.push("/cheiz/my-tours")}
            className="w-full py-3.5 rounded-2xl bg-cheiz-primary text-white font-bold shadow-lg shadow-cheiz-primary/25 active:scale-[0.98] transition-all"
          >
            마이페이지
          </button>
          <button
