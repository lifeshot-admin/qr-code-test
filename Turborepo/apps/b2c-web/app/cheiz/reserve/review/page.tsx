    ...
    try {
      ...
      if (editMode && existingReservationId) {
        ...
        if (deleteRes.ok) {
          ...
          console.log(`${getTimestamp()} ✅ [EDIT MODE] 기존 예약 상태 → CANCELED:`, delData);
        } else {
      ...
      try {
        await fetch("/api/bubble/notifications", {
          ...
          body: JSON.stringify({
            ...
            type: "POSE_COMPLETE",
            title: "포즈 예약이 완료되었습니다",
            body: tour?.tour_name || "포즈 예약",
            is_read: false,
          }),
        });
        console.log(`${getTimestamp()} ✅ POSE_COMPLETE 알림 생성 완료`);
      } catch (notifErr) {
        console.warn(`${getTimestamp()} ⚠️ 알림 생성 실패 (비치명적):`, notifErr);
      }
    ...
    <div className="min-h-screen bg-white pb-32">
      ...
      <div className="bg-white border-b border-gray-100">
        <div className="max-w-md mx-auto px-6 py-2 flex items-center gap-2 text-sm text-gray-500">
          <button
            onClick={handleBack}
            className="hover:text-cheiz-primary transition-colors"
          >
            ← 스팟 선택
          </button>
      ...
      <div className="max-w-md mx-auto px-6 py-8">
        <div className="mb-6">
          ...
          <p className="text-gray-600">
            총 <span className="font-bold text-cheiz-primary">{getTotalSelectedCount()}개</span>의 포즈를 선택하셨습니다 ✨
          </p>
        ...
        <div className="space-y-6 max-h-[calc(100vh-420px)] overflow-y-auto pr-2" style={{
          scrollbarWidth: 'thin',
          scrollbarColor: 'var(--cheiz-primary) #E5E7EB'
        }}>
            ...
            .map((spot) => (
              ...
              >
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className="bg-cheiz-primary text-white px-4 py-1 rounded-full text-sm font-medium">
                  ...
                  {spot.selectedPoses.map((poseId) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {pose?.persona && (
                          <div className="absolute top-2 right-2 bg-white bg-opacity-90 text-cheiz-primary px-2 py-1 rounded-full text-xs font-medium">
                            {pose.persona}
                          </div>
                        )}
                        <div className="absolute top-2 left-2 bg-cheiz-primary text-white rounded-full w-8 h-8 flex items-center justify-center text-sm font-bold">
                          ✓
                        </div>
                        {/* Hover overlay */}
          ...
          {emailVerified && !emailEditing ? (
            ...
            <div className="flex items-center gap-2">
              ...
              <button
                ...
                className="px-3 py-3 text-xs font-bold text-cheiz-primary bg-cheiz-primary/10 rounded-xl hover:bg-cheiz-primary/20 active:scale-95 transition-all"
              >
                변경
              </button>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  placeholder="이메일 주소"
                  className="flex-1 bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-700 border border-gray-200 outline-none focus:border-cheiz-primary transition-colors"
                />
                <button
                  onClick={sendVerificationCode}
                  disabled={codeSending || !emailInput.includes("@")}
                  className="px-4 py-3 text-xs font-bold text-white bg-cheiz-primary rounded-xl disabled:opacity-40 active:scale-95 transition-all whitespace-nowrap"
                >
                  ...
                  >
                    <div className="flex gap-2">
                      <input
                        ...
                        className="flex-1 bg-gray-50 rounded-xl px-4 py-3 text-sm text-gray-700 border border-gray-200 outline-none focus:border-cheiz-primary transition-colors text-center tracking-[6px] font-bold"
                      />
                      <button
                        onClick={verifyCode}
          ...
          <button
            ...
            className={`w-full py-4 rounded-2xl font-bold text-base transition-all ${
              validation?.canProceedToReview && emailVerified
                ? "bg-cheiz-primary text-white shadow-lg shadow-cheiz-primary/25 active:scale-[0.98]"
          ...
          >
            ...
            >
              ...
              <button
                ...
                className="absolute -top-12 right-0 text-white text-4xl hover:text-cheiz-primary transition-colors z-10"
              >
                ✕
              </button>
              ...
              {/* Persona badge */}
              {lightboxPersona && (
                <div className="absolute top-4 right-4 bg-cheiz-primary text-white px-4 py-2 rounded-full font-medium shadow-lg">
                  {lightboxPersona}
                </div>
              )}
        ...
        {showSuccessModal && (
          ...
          >
            ...
            >
              ...
              <h2 className="text-3xl font-bold text-cheiz-primary mb-4">
                예약 완료!
              </h2>
              ...
              {/* 6자리 예약 코드 (크게 표시) */}
              {reservationCode && (
                <div className="bg-gradient-to-r from-cheiz-primary/10 to-[#7B2BFF]/10 border-2 border-cheiz-primary/20 rounded-2xl p-5 mb-6">
                  <p className="text-xs text-gray-500 font-semibold mb-2 tracking-wide uppercase">예약 번호</p>
                  <p className="text-4xl font-mono font-extrabold tracking-[0.3em] text-cheiz-primary">
                    {reservationCode}
                  </p>
