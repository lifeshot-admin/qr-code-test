...
function SpotsContent() {
  ...
  const handleUpdatePoses = async (safeTourId: number) => {
    ...
    try {
      ...
      if (!deleteRes.ok) {
        console.warn("[EDIT_MODE] ⚠️ 기존 예약 상태 변경 실패, 새로 생성 시도...");
      } else {
      ...
      <div className="min-h-screen bg-white flex items-center justify-center p-6">
        ...
        >
          ...
          <button
            ...
            className="bg-cheiz-primary text-white font-bold py-4 px-8 rounded-xl hover:bg-opacity-90 transition-all transform hover:scale-105 shadow-sm"
          >
            쿠폰 조회하기
          </button>
        </motion.div>
                  ...
                  <div className="bg-white rounded-xl p-2 pb-4 shadow-md hover:shadow-xl transition-shadow duration-300 border border-gray-100/80">
                    ...
                    <div className="pt-2.5 px-1">
                      ...
                      {spotValidation?.message ? (
                        ...
                        <p className="text-xs text-cheiz-primary mt-0.5 font-medium">
                          포즈 고르기 &rarr;
                        </p>
                      )}
                    </div>
