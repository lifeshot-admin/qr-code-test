    ...
    return (
      <div className="min-h-screen bg-[#FAFAF8] flex items-center justify-center">
        <div className="text-center">
          ...
          <button onClick={() => router.back()} className="mt-4 text-cheiz-primary font-bold text-sm">돌아가기</button>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FAFAF8] pb-24">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.05 }}
          ...
          <div className="flex flex-wrap gap-2 mt-4">
            {retoucher.specialties.map(s => (
              <span key={s} className="text-[11px] font-bold px-3 py-1 rounded-full bg-cheiz-primary/5 text-cheiz-primary border border-cheiz-primary/10">
                {s}
              </span>
            ))}
          </div>
              ...
              {retoucher.beforeAfterSamples.map((_, i) => (
                <button key={i} onClick={() => { setBaIndex(i); setSliderPos(50); }}
                  className={`px-2.5 py-1 rounded-lg text-[10px] font-bold transition-all ${
                    baIndex === i ? "bg-cheiz-primary text-white" : "bg-gray-100 text-gray-400"
                  }`}>
                  {i + 1}
                </button>
              ))}
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 z-50 pb-[env(safe-area-inset-bottom)]">
        <div className="max-w-md mx-auto px-5 py-3">
          <button onClick={() => router.back()}
            className="w-full h-12 bg-cheiz-primary text-white text-sm font-bold rounded-xl active:scale-[0.98] transition-all shadow-lg shadow-cheiz-primary/20 flex items-center justify-center gap-2">
