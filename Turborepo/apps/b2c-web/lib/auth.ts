...
/**
 * role 값을 항상 문자열로 정규화
 * 백엔드가 객체({ authority: "ROLE_USER" }), 배열(["ROLE_USER"]), 또는 문자열을 반환할 수 있음
 */
function normalizeRole(raw: unknown): string {
  ...
  if (Array.isArray(raw)) {
    ...
    if (typeof first === "object" && first !== null) {
      return String(first.authority || first.role || first.name || first.value || "");
    }
    return String(first);
  }
  if (typeof raw === "object" && raw !== null) {
    const obj = raw as Record<string, unknown>;
    return String(obj.authority || obj.role || obj.name || obj.value || "");
  }
  return String(raw);
}
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        try {
          ...
          let role = normalizeRole(userData.role) || "User";
          ...
          try {
            ...
            if (meRes.ok) {
              ...
              role = normalizeRole(realData.role) || role;
        ...
        try {
          ...
          try {
            ...
            if (userMeResponse.ok) {
              ...
              // ✅ role 필드 광범위 탐색 + 문자열 정규화
              realRole = normalizeRole(meData.role)
                || normalizeRole(meData.userRole)
                || normalizeRole(meData.authority)
                || normalizeRole(meData.user_role)
                || normalizeRole(meData.type)
                || normalizeRole(meData.authorities)
                || normalizeRole(meData.roles)
                || null;
          ...
          if (!realRole) {
            realRole = normalizeRole(userData.role)
              || normalizeRole(userData.userRole)
              || normalizeRole(userData.authority)
              || normalizeRole(userData.user_role)
              || normalizeRole(userData.type)
              || normalizeRole(userData.authorities)
              || normalizeRole(userData.roles)
              || null;
          }
  ...
  callbacks: {
            ...
            if (finalToken) {
              ...
              try {
                ...
                if (userMeResponse.ok) {
                  ...
                  (user as any).role = normalizeRole(realData.role) || "User";
                  ...
                  (user as any).role = normalizeRole(userData.role) || "User";
    ...
    async session({ session, token }) {
      if (session.user) {
        ...
        // ✅ role: JWT 토큰에서 세션으로 확실히 전달 (객체 방어)
