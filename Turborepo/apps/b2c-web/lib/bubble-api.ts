...
export async function fetchReviewById(id: string): Promise<BubbleReview | null> {
  if ((!API_BASE_URL && !APP_NAME) || !API_TOKEN) return null;
  try {
    const url = `${BASE}/review/${id}`;
    const res = await bubbleFetch(url, { method: "GET", headers: headers() });
    if (!res.ok) {
      console.error(`[fetchReviewById] HTTP ${res.status}`);
      return null;
    }
    const json = await res.json();
    return json?.response || null;
  } catch (e) {
    console.error("[fetchReviewById] Exception:", e);
    return null;
  }
}

export type BubbleReviewImage = {
  _id: string;
  review_Id?: string;
  image?: string;
  order?: number;
...
export async function fetchReviewImagesByReviewId(
  reviewId: string
): Promise<BubbleReviewImage[]> {
  if ((!API_BASE_URL && !APP_NAME) || !API_TOKEN) return [];
  try {
    const constraints = JSON.stringify([
      { key: "review_Id", constraint_type: "equals", value: reviewId },
    ]);
    const url = `${BASE}/review_image?constraints=${encodeURIComponent(constraints)}&sort_field=order&descending=false`;
    const res = await bubbleFetch(url, { method: "GET", headers: headers() });
    if (!res.ok) {
      console.error(`[fetchReviewImagesByReviewId] HTTP ${res.status}`);
      return [];
    }
    const json = await res.json();
    return json?.response?.results ?? [];
  } catch (e) {
    console.error("[fetchReviewImagesByReviewId] Exception:", e);
    return [];
  }
}

export async function fetchReviewsByAlbumId(
  albumId: string,
  statusFilter?: string
): Promise<BubbleReview[]> {
  if ((!API_BASE_URL && !APP_NAME) || !API_TOKEN) return [];
  try {
    ...
    if (statusFilter) {
      constraints.push({ key: "color_grade_status", constraint_type: "equals", value: statusFilter });
    }
    const url = `${BASE}/review?constraints=${encodeURIComponent(JSON.stringify(constraints))}`;
    const res = await bubbleFetch(url, { method: "GET", headers: headers() });
    if (!res.ok) {
      console.error(`[fetchReviewsByAlbumId] HTTP ${res.status}`);
      return [];
    }
    const json = await res.json();
    return json?.response?.results ?? [];
  } catch (e) {
    console.error("[fetchReviewsByAlbumId] Exception:", e);
    return [];
  }
}
