  ...
  if (loading) {
    return (
      <div className="flex items-center gap-2 py-3">
        <div className="w-4 h-4 rounded-full border-2 border-gray-200 border-t-[#0055FF] animate-spin" />
  ...
  return (
    <div className={`grid grid-cols-3 gap-2 ${compact ? "" : "mb-4"}`}>
      ...
      <div className="bg-blue-50 rounded-xl p-3 text-center">
        <Download className="w-4 h-4 text-[#0055FF] mx-auto mb-1" />
        <p className="text-xl font-bold text-[#0055FF]">{photo}</p>
        <p className="text-[10px] text-gray-500 mt-0.5 leading-tight">사진 다운로드</p>
      </div>

      {/* AI — 인라인 적용 버튼 통합 */}
      <div className="bg-purple-50 rounded-xl p-3 text-center relative">
...
export function GiftCouponCard({
  ...
  if (variant === "compact") {
    return (
      <div className="flex-shrink-0 w-52 bg-gradient-to-br from-white to-blue-50 border border-[#0055FF]/10 rounded-xl p-3.5 text-left hover:shadow-md hover:border-[#0055FF]/30 transition-all">
        <div className="flex items-center gap-1.5 mb-1.5">
          <Gift className="w-3.5 h-3.5 text-[#0055FF]" />
          <p className="text-[10px] font-bold text-[#0055FF] truncate">{name}</p>
        </div>
        ...
        <div className="flex gap-1 mb-2 flex-wrap">
          {photoCredits > 0 && (
            <span className="inline-flex items-center gap-0.5 text-[9px] font-bold text-[#0055FF] bg-blue-50 px-1.5 py-0.5 rounded-md">
              <Download className="w-2.5 h-2.5" />{photoCredits}
            </span>
          )}
          {aiCredits > 0 && (
            <span className="inline-flex items-center gap-0.5 text-[9px] font-bold text-purple-600 bg-purple-50 px-1.5 py-0.5 rounded-md">
        ...
        {onAction && actionLabel && (
          <button
            ...
            disabled={loading}
            className="w-full py-1.5 bg-[#0055FF] text-white text-[10px] font-bold rounded-lg hover:bg-opacity-90 disabled:opacity-40 transition-all active:scale-95 flex items-center justify-center gap-1"
          >
            {loading ? "..." : actionLabel}
          </button>
        )}
  ...
  return (
    ...
    >
      {/* 상단 바 */}
      <div className="bg-gradient-to-r from-[#0055FF] to-[#3377FF] px-5 py-3 flex items-center justify-between">
      ...
      <div className="p-4">
        ...
        <div className="flex gap-2 mb-3">
          {photoCredits > 0 && (
            <div className="flex items-center gap-1.5 bg-blue-50 rounded-lg px-3 py-2">
              ...
              <span className="text-xs font-bold text-[#0055FF]">{photoCredits}</span>
          ...
          {aiCredits > 0 && (
            <div className="flex items-center gap-1.5 bg-purple-50 rounded-lg px-3 py-2">
              ...
              <span className="text-xs font-bold text-purple-600">{aiCredits}</span>
        ...
        {onAction && actionLabel && (
          <button
            onClick={onAction}
            disabled={loading}
            className="w-full py-2.5 bg-[#0055FF] text-white text-sm font-bold rounded-xl hover:bg-opacity-90 disabled:opacity-40 transition-all active:scale-[0.98]"
          >
