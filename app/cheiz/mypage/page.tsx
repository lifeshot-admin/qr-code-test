      ...
      <div className="min-h-screen bg-[#FAFAF8] flex items-center justify-center p-6">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-sm w-full text-center">
          ...
          <button onClick={() => router.push("/auth/signin?callbackUrl=/cheiz/mypage")}
            className="w-full bg-[#0055FF] text-white font-bold py-4 rounded-2xl hover:bg-opacity-90 transition-all shadow-sm active:scale-[0.98]">로그인 하기</button>
        </motion.div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-[#FAFAF8] pb-24">
      ...
      <div className="bg-white border-b border-gray-100 sticky top-0 z-50">
        <div className="max-w-md mx-auto px-5 py-3 flex items-center justify-between">
          <button onClick={() => router.push("/cheiz")} className="text-gray-500 hover:text-[#0055FF] transition-colors text-sm flex items-center gap-1 active:scale-95">
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
          <div className="flex items-center gap-4">
            <button onClick={() => fileInputRef.current?.click()}
              className="relative w-16 h-16 rounded-full overflow-hidden bg-gradient-to-br from-[#0055FF] to-[#3377FF] flex items-center justify-center flex-shrink-0 active:scale-95 transition-transform">
                ...
                <div className="space-y-1.5">
                  <div className="flex gap-2">
                    <input type="text" value={nicknameInput} autoFocus
                      ...
                      className="flex-1 text-lg font-bold text-gray-900 border-b-2 border-[#0055FF] bg-transparent outline-none px-0 py-1" placeholder="새 닉네임" />
                    <button onClick={saveNickname} disabled={nicknameSaving || nicknameAvailable === false || !nicknameInput.trim()}
                      className="px-3 py-1 bg-[#0055FF] text-white text-xs font-bold rounded-lg disabled:opacity-40 active:scale-95">
